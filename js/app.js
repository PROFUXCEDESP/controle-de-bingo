document.addEventListener("DOMContentLoaded", () => {
    ...
    // CONTROLE DE PAGINAÇÃO SEGURO
    window.pages = {
        rankEdu: { current: 1, term: "" },
        rankAdm: { current: 1, term: "" },
        lotes: { current: 1, term: "" },
        logs: { current: 1 }
    };
    ...
    // UI PAGINAÇÃO
    // ==========================================
    function renderPaginationUI(containerId, key, totalItems, itemsPerPage, renderFunc) {
        ...
        container.innerHTML = `
            <button class="btn-page" ${window.pages[key].current === 1 ? 'disabled' : ''} onclick="window.goPage('${key}', -1, '${renderFunc}')"><span class="material-symbols-outlined">chevron_left</span></button>
            <span class="page-counter">Pág. <input type="number" class="page-jump" value="${window.pages[key].current}" min="1" max="${totalPages}" onchange="window.jumpPage('${key}', this, '${renderFunc}')"> de ${totalPages}</span>
            <button class="btn-page" ${window.pages[key].current === totalPages ? 'disabled' : ''} onclick="window.goPage('${key}', 1, '${renderFunc}')"><span class="material-symbols-outlined">chevron_right</span></button>
        `;
    }
    ...
    // ==========================================
    // SELECTS CUSTOMIZADOS E BUSCAS INLINE
    // ==========================================
        ...
        btnConfirmarCorte.addEventListener('click', (e) => {
            ...
            if(cropperInstancia) {
                if (window.modoEdicaoFoto && window.alunoEmFocoIdx !== null) {
                    btnConfirmarCorte.innerText = "Enviando Foto..."; btnConfirmarCorte.disabled = true;
    ...
    // ==========================================
    // RENDERIZADORES DE TABELA PAGINADA
    // ==========================================
    window.renderRankingAlunosEdu = function() {
        const tabela = document.getElementById("tabelaRankingEducador"); if(!tabela) return;
        const searchInput = document.getElementById('buscaRankingAluno');
        if(searchInput) window.pages.rankEdu.term = searchInput.value;
        ...
        tabela.innerHTML = paginated.map((aluno, index) => {
            ...
            return `<tr style="${destaque}" onclick="abrirDetalhesAluno(${window.todosEducandosBD.indexOf(aluno)})">
                <td class="td-center" style="font-weight: bold;">${posReal}º</td>
                <td><img src="${aluno.foto}" class="table-avatar"></td>
                <td><strong style="color:var(--obsidian);">${aluno.nome}</strong>${tagMeu}</td>
    ...
    window.renderRankingAlunosAdm = function() {
        ...
        const searchInput = document.getElementById('buscaRankingAluno');
        if(searchInput) window.pages.rankAdm.term = searchInput.value;
        ...
        tabela.innerHTML = paginated.map((aluno, index) => {
            ...
            return `<tr onclick="abrirDetalhesAluno(${window.todosEducandosBD.indexOf(aluno)})">
                <td class="td-center" style="font-weight: bold;">${posReal}º</td>
                <td><img src="${aluno.foto}" class="table-avatar"></td>
                <td><strong style="color:var(--obsidian);">${aluno.nome}</strong>${tagMeu}</td>
                <td style="color:var(--dim-grey);">${aluno.turma}</td>
    ...
    window.renderGestaoLotesPaginado = function() {
        ...
        const searchInput = document.getElementById('buscaGestaoLotes');
        if(searchInput) window.pages.lotes.term = searchInput.value;

        let filtrados = window.todosEducandosBD.filter(a => a.lotesPendentes.length > 0);
        if(window.pages.lotes.term) filtrados = filtrados.filter(a => a.nome.toLowerCase().includes(window.pages.lotes.term.toLowerCase()));

        const startIdx = (window.pages.lotes.current - 1) * 10;
        const paginated = filtrados.slice(startIdx, startIdx + 10);
        ...
        renderPaginationUI('pagLotes', 'lotes', filtrados.length, 10, 'renderGestaoLotesPaginado');
    }
    ...
    if(srchRankA) srchRankA.addEventListener('input', () => { window.pages.rankAdm.current = 1; window.pages.rankEdu.current = 1; document.getElementById("adminPage") ? window.renderRankingAlunosAdm() : window.renderRankingAlunosEdu(); });
    
    const srchLotes = document.getElementById("buscaGestaoLotes");
    if(srchLotes) srchLotes.addEventListener('input', () => { window.pages.lotes.current = 1; window.renderGestaoLotesPaginado(); });
    ...
    if (educadorPage) {
        ...
        window.atualizarDashboardEducador = function() {
            ...
            if (tabela) {
                tabela.innerHTML = meusAlunosAtivos.map((aluno, index) => {
                    const rec = calcularRecompensas(aluno.lotesVendidos.length);
                ...
                }).join('') || '<tr><td colspan="8" class="text-center" style="padding: 20px; color: #a0a0a0;">Nenhum aluno ativo encontrado.</td></tr>';
            }
            ...
            if(tabelaRankingEducadores) {
                ...
                tabelaRankingEducadores.innerHTML = rankingProf.map((prof, index) => {
                    ...
                    return `<tr style="${destaque}"><td class="td-center" style="font-weight: bold; color: var(--petal-pink);">${index + 1}º</td><td><strong>${prof.nome}</strong></td><td style="color: var(--dim-grey);">${prof.curso}</td><td class="td-center" style="font-weight: bold; font-size: 1.1rem;">${prof.lotesVendidos}</td><td class="td-center highlight-purple" style="font-weight: bold;">R$ ${valorArrecadado}</td></tr>`;
                }).join('');
            }
    ...
    // ==========================================
    // LÓGICA DO ADM E CADASTRO DE EDUCADOR
    // ==========================================
    ...
    if (adminPage) {
        ...
        if(formCadastrarEducador) {
            formCadastrarEducador.addEventListener("submit", (e) => {
                e.preventDefault();
                const nomeEdu = document.getElementById("novoNomeEducador").value;
                const cursoEdu = document.getElementById("novoCursoEducador").value;
                const btn = formCadastrarEducador.querySelector("button[type='submit']");
                btn.innerText = "Salvando..."; btn.disabled = true;

                fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'cadastrar_educador', nome: nomeEdu, curso: cursoEdu }) })
                .then(res => res.json()).then(data => {
                    btn.innerText = "Salvar Educador"; btn.disabled = false;
                    if(data.success) {
                        window.registrarLog("Cadastro Educador", `Cadastrou o novo educador: ${nomeEdu}`);
                        fecharModal('modalCadastrarEducador'); formCadastrarEducador.reset();
                        window.abrirModalSucesso(data.message); window.carregarDadosDoBanco(); 
                    } else { window.abrirModalErro(data.message); }
                }).catch(err => { btn.disabled = false; btn.innerText = "Salvar Educador"; window.abrirModalErro("Erro de rede."); });
            });
        }
        ...
        window.atualizarDashboardsADM = function() {
            ...
            if(tabelaRankingEducadoresADM) {
                let rankingProf = [...window.mockEducadoresBD].sort((a, b) => b.lotesVendidos - a.lotesVendidos);
                tabelaRankingEducadoresADM.innerHTML = rankingProf.map((prof, index) => {
                    const valorArrecadado = (prof.lotesVendidos * 20).toFixed(2).replace('.', ',');
                    return `<tr><td class="td-center" style="font-weight: bold; color: var(--petal-pink);">${index + 1}º</td><td><strong>${prof.nome}</strong></td><td style="color: var(--dim-grey);">${prof.curso}</td><td class="td-center" style="font-weight: bold; font-size: 1.1rem;">${prof.lotesVendidos}</td><td class="td-center highlight-purple" style="font-weight: bold;">R$ ${valorArrecadado}</td></tr>`;
                }).join('');
            }
        ...
        if(formTransferirLote) {
            formTransferirLote.addEventListener("submit", (e) => {
                ...
                window.registrarLog("Transferência", `Moveu lotes ${lotesSel.join(', ')} para o destino: ${dest}`);
    ...
    window.registrarRetiradaCartela = function(idx) {
        ...
        window.registrarLog("Retirada Cartela", `O Aluno ${aluno.nome} retirou +1 cartela física.`);
    ...
    window.confirmarVendaLote = function() {
        ...
        window.registrarLog("Venda", `Lote ${lote} validado para ${aluno.nome}. Pagamento: R$ ${vPix+vDin} via ${formaPagamento}`);
        if(document.getElementById("adminPage")) window.atualizarDashboardsADM(); 
        if(document.getElementById("educadorPage")) window.atualizarDashboardEducador();
